package com.aditya.functionalProgramming.test;
...
public class Challenge1 {
	...
	public static void main(String[] args) {
		...
		// Answer 2 goes here
		List<String> getBlueCars = cars.stream()
									    .filter(x -> x.color.equals("blue"))
										.map(x -> x.make)
										.collect(Collectors.toList());
		System.out.println(getBlueCars);
		...
		// Answer 3 goes here
		BinaryOperator<Float> sumSalary = (acc, x) -> {
			return acc + x;
		};
		Float salarySum = employees.stream()
								   .map(x -> x.salary)
								   .reduce(0f, sumSalary);
		System.out.println(salarySum);
		
		Float developerSalaryTotal = employees.stream()
											  .filter(x -> x.jobTitle.equals("developer"))
											  .map(x -> x.salary)
											  .reduce(0f, (acc, x) -> acc + x);
		
		System.out.println(developerSalaryTotal);
